	...
	class EmergencyMinigameUpdatePatch {
		static void Postfix(EmergencyMinigame __instance) {
			...
			if (Lawyer.lawyer != null && Lawyer.lawyer == PlayerControl.LocalPlayer && Lawyer.winsAfterMeetings) {
				roleCanCallEmergency = false;
				statusText = "弁護士は緊急会議を開始出来ません (" + Lawyer.meetings + "/" + Lawyer.neededMeetings + " meetings)";
			}
			...
			static bool Prefix(MapCountOverlay __instance) {
				...
				for (int i = 0; i < __instance.CountAreas.Length; i++)
				{
					...
					if (!commsActive)
					{
						...
						{
							Debug.LogWarning("カウンターが見つかりませんでした:" + counterArea.RoomType);
						}
					}
					else
					{

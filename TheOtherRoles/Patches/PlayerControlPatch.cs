        ...
        static void Postfix(PlayerControl __instance, [HarmonyArgument(0)]GameData.PlayerInfo target)
        {
            ...
            if (isMedicReport || isDetectiveReport)
            {
                ...
                if (deadPlayer != null && deadPlayer.killerIfExisting != null) {
                    ...
                    if (isMedicReport)
                    {
                        msg = $"éÄëÃïÒçê: âΩ {Math.Round(timeSinceDeath / 1000)} ïbëOÇ…éEÇ≥ÇÍÇ‹ÇµÇΩ";
                    }
                    else if (isDetectiveReport)
                    {
                        if (timeSinceDeath < Detective.reportNameDuration * 1000)
                        {
                            msg = $"éÄëÃïÒçê: éEäQé“ÇÕ {deadPlayer.killerIfExisting.Data.PlayerName} Ç≈Ç∑";
                        }
                        else if (timeSinceDeath < Detective.reportColorDuration * 1000)
                        {
                            var typeOfColor = Helpers.isLighterColor(deadPlayer.killerIfExisting.Data.DefaultOutfit.ColorId) ? "íWêF" : "îZêF";
                            msg = $"éÄëÃïÒçê: éEäQé“ÇÕ {typeOfColor}êFÇ≈Ç∑";
                        }
                        else
                        {
                            msg = $"éÄëÃïÒçê: éÄëÃÇÕå√Ç∑Ç¨ÇƒèÓïÒÇìæÇÈéñÇ™èoóàÇ‹ÇπÇÒ";
                        }
                    }

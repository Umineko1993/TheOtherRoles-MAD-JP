                ...
                if (AmongUsClient.Instance.AmHost) {
                    ...
                    foreach (InnerNet.ClientData client in AmongUsClient.Instance.allClients.ToArray()) {
                        ...
                        if (dummyComponent != null && dummyComponent.enabled)
                            ...
                            if (diff > 0) {
                                message += $"<color=#FF0000FF>{client.Character.Data.PlayerName} は古いバージョン (v{playerVersions[client.Id].version.ToString()}) が導入されています\n</color>";
                                blockStart = true;
                            } else if (diff < 0) {
                                message += $"<color=#FF0000FF>{client.Character.Data.PlayerName} には新しいバージョン (v{playerVersions[client.Id].version.ToString()}) があります\n</color>";
                                blockStart = true;
                            } else if (!PV.GuidMatches()) { // version presumably matches, check if Guid matches
                                message += $"<color=#FF0000FF>{client.Character.Data.PlayerName} 修正バージョン (v{playerVersions[client.Id].version.ToString()} <size=30%>({PV.guid.ToString()}) </size>があります\n</color>";
                                blockStart = true;
                            }
                        }
                    }

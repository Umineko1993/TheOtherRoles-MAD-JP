        ...
        public static void Postfix(HudManager __instance)
        {
            ...
            mediumButton = new CustomButton(
                ...
                () => {
                    ...
                    string typeOfColor = Helpers.isLighterColor(Medium.target.killerIfExisting.Data.DefaultOutfit.ColorId) ? "íWêF" : "îZêF";
                    float timeSinceDeath = ((float)(Medium.meetingStartTime - Medium.target.timeOfDeath).TotalMilliseconds);
                    string name = " (" + Medium.target.player.Data.PlayerName + ")";


                    if (randomNumber == 0) msg = "Ç†Ç»ÇΩÇÃñêEÇÕâΩÇ≈Ç∑Ç©? éÑÇÃñêEÇÕ " + RoleInfo.GetRolesString(Medium.target.player, false) + "Ç≈Ç∑" + name;
                    else if (randomNumber == 1) msg = "Ç†Ç»ÇΩÇéEäQÇµÇΩî∆êlÇÃêFÇÕâΩÇ≈Ç∑Ç© ? éÑÇéEäQÇµÇΩî∆êlÇÃêFÇÕ " + typeOfColor + "Ç≈Ç∑" + name;
                    else if (randomNumber == 2) msg = "Ç†Ç»ÇΩÇÕÇ¢Ç¬éÄñSÇµÇ‹ÇµÇΩÇ©? éÑÇÕ " + Math.Round(timeSinceDeath / 1000) + "âÒñ⁄ÇÃâÔãcäJénëOÇ…éÄñSÇµÇ‹ÇµÇΩ" + name;
                    else msg = "Ç†Ç»ÇΩÇéEäQÇµÇΩî∆êlÇÃñêEÇÕâΩÇ≈Ç∑Ç©? éÑÇéEäQÇµÇΩî∆êlÇÃñêEÇÕ " + RoleInfo.GetRolesString(Medium.target.killerIfExisting, false) + "Ç≈Ç∑" + name; //exlude mini

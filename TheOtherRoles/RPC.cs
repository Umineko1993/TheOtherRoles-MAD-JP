	...
	public static class RPCProcedure {
		...
		public static void ShareOptions(int numberOfOptions, MessageReader reader) {            
			try {
				...
				TheOtherRolesPlugin.Logger.LogError("オプションの逆シリアル化中にエラーが発生しました: " + e.Message);
			}
		}
		...
		public static void lightsOut() {
			...
			if(PlayerControl.LocalPlayer.Data.Role.IsImpostor) {
				new CustomMessage("トリックスター停電中", Trickster.lightsOutDuration);
			}
		}
		...
		public static void placeCamera(byte[] buff) {
			...
			camera.CamName = $"セキュリティーカメラ {SecurityGuard.placedCameras}";
		...
		public static void guesserShoot(byte dyingTargetId, byte guessedTargetId, byte guessedRoleId) {
			...
			if (Guesser.showInfoInGhostChat && PlayerControl.LocalPlayer.Data.IsDead && guessedTarget != null) {
				...
				string msg = $"ゲッサーは {guessedTarget.Data.PlayerName} を {roleInfo?.name ?? ""} と推測しました";
